...
export function WebAppDemo() {
  ...
  return (
    <div ref={containerRef} className="w-full h-full p-4 md:p-6 relative">
      ...
      >
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/2 h-1/2 bg-emerald-500/20 rounded-full" />
      </motion.div>
    </div>
  );
}
      ...
      >
        ...
        <div className="mb-2">
          ...
          <motion.div 
            ...
            animate={phase >= 1 ? { borderColor: "rgba(16, 185, 129, 0.3)" } : {}}
          >
            <motion.span 
            ...
            {phase === 1 && (
              <motion.span 
                className="w-0.5 h-3 bg-emerald-400 ml-0.5"
                animate={{ opacity: [1, 0] }}
        ...
        {/* Login button */}
        <motion.div
          className="h-7 bg-gradient-to-r from-emerald-500 to-teal-500 rounded flex items-center justify-center cursor-pointer"
          animate={phase >= 3 ? { scale: [1, 0.95, 1] } : {}}
          transition={{ duration: 0.2 }}
        >
          <span className="text-[9px] font-medium text-white">Sign In</span>
        </motion.div>
...
function Dashboard({ phase }: { phase: number }) {
  return (
    ...
    >
      ...
      >
        {/* Logo */}
        <div className="w-full aspect-square rounded-lg bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center mb-2">
          <div className="w-4 h-4 border-2 border-white rounded" />
        </div>
        ...
        {[true, false, false, false].map((active, i) => (
          ...
          >
            <div className={`w-3 h-3 rounded ${active ? "bg-emerald-400" : "bg-white/20"}`} />
          </motion.div>
        ))}
      </motion.div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 p-3 md:p-4 space-y-3 overflow-hidden">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: "Revenue", value: "$24.5k", color: "emerald" },
              { label: "Users", value: "1,429", color: "teal" },
              { label: "Growth", value: "+12.3%", color: "cyan" },
            ].map((stat, i) => (
              <motion.div
                key={i}
              ...
              >
                ...
                <motion.div
                  className={`h-0.5 mt-1 rounded-full bg-gradient-to-r ${
                    ...
                    stat.color === "teal" ? "from-teal-500 to-teal-400" :
                ...
                {["1D", "1W", "1M"].map((t, i) => (
                  <div 
                    ...
                    className={`px-1.5 py-0.5 rounded text-[6px] ${
                      i === 2 ? "bg-emerald-500/20 text-emerald-400" : "text-white/30"
                    }`}
                  >
                    {t}
                  </div>
